  ...
  return (
    <PageShell>
      <PageHero
        ...
        imageAlt="Zexa Media marketing dashboard with campaign analytics and growth metrics"
        metricLabel="Growth insights"
        metricValue="Ads, automation, reporting"
      />
        ...
        <StaggerContainer className="grid gap-5 lg:grid-cols-3">
          {posts.map((post) => (
            <StaggerItem key={post.href}>
              <Link
                href={post.href}
                className="group flex min-h-full flex-col overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.045] transition duration-300 hover:-translate-y-1 hover:border-[#8EEA4D]/35 hover:bg-white/[0.06]"
              >
                <div className="relative overflow-hidden border-b border-white/10 bg-[#071006] p-6">
                  <div className="absolute inset-0 opacity-70 [background-image:radial-gradient(circle_at_25%_20%,rgba(142,234,77,0.16),transparent_30%),linear-gradient(135deg,rgba(255,255,255,0.06),transparent)] transition duration-300 group-hover:scale-[1.025]" />
                  <div className="relative grid min-h-32 place-items-center rounded-[1.25rem] border border-white/10 bg-black/20">
                    <post.icon className="size-11 text-[#8EEA4D] transition duration-300 group-hover:scale-[1.05]" strokeWidth={1.7} />
                  </div>
                </div>
